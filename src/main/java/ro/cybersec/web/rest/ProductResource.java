...
/**
 * REST controller for managing {@link ro.cybersec.domain.Product}.
 */
@RestController
@RequestMapping("/api")
public class ProductResource {
     ...
     */
    @GetMapping("/products")
    public ResponseEntity<List<ProductDTO>> getAllProducts(Pageable pageable, @RequestParam(required = false) String filter, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        if ("productordered-is-null".equals(filter)) {
            log.debug("REST request to get all Products where productOrdered is null");
            return new ResponseEntity<>(productService.findAllWhereProductOrderedIsNull(),
                    HttpStatus.OK);
        }
